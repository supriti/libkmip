                    ...
                    char ***located_item_uuids)
{
    KMIP ctx[1] = {{0}};
    kmip_init(ctx, NULL, 0, KMIP_1_0);
    int result = kmip_bio_locate_with_context(ctx, bio, attributes, attribute_count, located_item_count, located_item_uuids);
    kmip_destroy(ctx);
    return result;
}
                                 ...
                                 char ***located_item_uuids)
{
    ...
    char **result_id = ctx->calloc_func(ctx->state, pld->unique_identifiers_count + 1, sizeof(char *));
